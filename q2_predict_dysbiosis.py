...
# Suppress the specific warning about missing feature names - to test on any data without the need to modify
...
def calculate_index(table, pathways_stratified, pathways_unstratified, mode):
    ...
    if mode == "no_taxonomy":
        ml_model = joblib.load("data/q2_pd_model_no_gmhi.pkl")
        ...
        for sample in sample_list:
            ...
            try:
                ...
                if len(contributions_count_nonneg) > 0:
                ...
                if len(sample_row) == 5:
                    ...
                    frac_core_fun_col.append(sample_row[1])
                    #print(frac_core_fun_col)
                    frac_core_found_col.append(sample_row[2])
                    #print(frac_core_found_col)
                    spec_found_tog_col.append(sample_row[3])
                    #print(spec_found_tog_col)
                    contr_per_spec_col.append(sample_row[4])
        ...
        ml_model = joblib.load("data/q2_pd_model.pkl")
        # Keep rows as samples, columns as taxonomical species names
        table_df = table
        pathways_stratified_df = pathways_stratified
        pathways_unstratified_df = pathways_unstratified
