...
def calculate_index(table, pathways_stratified, pathways_unstratified):
    ...
    core = list(pd.read_csv("Q2PD_data/core_functions.txt", sep="\t").iloc[:,0])
    positive_pairs = list(pd.read_csv("Q2PD_data/positive_pairs.txt", sep="\t").iloc[:,0])
    ...
    params_df = pd.DataFrame(columns=["Sample","GMHI_good","GMHI_bad","Frac_of_core_functions_among_all","Frac_of_core_functions_found","Species_found_together","Contributions_per_species"])
    sample_col = []
    gupta_good_col = []
    gupta_bad_col = []
    frac_core_fun_col = []
    ...
    for sample in sample_list:
        ...
        try:
            ...
            sample_paths_unstrat = sample_paths_unstrat_cumsum.loc[sample_paths_unstrat_cumsum['cumsum'] > 0.001].drop('cumsum', axis=1)
            ...
            sample_paths_strat = sample_paths_strat_cumsum.loc[sample_paths_strat_cumsum['cumsum'] > 0.0001].drop('cumsum', axis=1)
    ...
    params_df["GMHI_good"] = gupta_good_col
    params_df["GMHI_bad"] = gupta_bad_col
...
taxa = pd.read_csv("Data/merged_taxonomy.txt", sep="\t", index_col=0)
paths_strat = pd.read_csv("Data/merged_stratified_pathways.txt", sep="\t", index_col=0)
paths_unstrat = pd.read_csv("Data/merged_unstratified_pathways.txt", sep="\t", index_col=0)


results = calculate_index(taxa, paths_strat, paths_unstrat)
results.to_csv('Data/calculated_parameters_for_merged.txt', sep="\t")